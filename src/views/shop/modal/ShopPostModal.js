...
export default function ShopPostModal(props) {
	...
	const flagSlice_Nations = "shop_Nations";
	const api_Nations = "/Nations";
	const api = "/User";

	const [formdata, setFormdata] = useState({code: '', nome: '', addr: '', zip: '', price_ship: '', phone: ''}); // 创建的数据
	
	const [farQuery_Shops, setfarQuery_Shops] = useState({});		// 是否有店铺选项
	const [isShop, setIsShop] = useState(false);		// 是否有店铺选项
	const Shops = useSelector(selectObjects(flagSlice_Nations));
	//
